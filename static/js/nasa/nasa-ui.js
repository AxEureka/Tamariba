...
export function createItemEditor(container,onSubmit){
    ...
    function buildItems(){
        ...
        for(let i=0;i<n;i++){
            ...
            first.value=""; first.textContent="選択"; first.disabled=true; first.selected=true;
...
export function createRankingUI(container, items, onSubmit, title, isTeam=false, isLeader=true){
    ...
    function checkDuplicate(){
        const values = selects.map(s => s.value).filter(v => v !== "");
        let dup = [];
        values.forEach(v=>{ if(values.filter(x=>x===v).length > 1) dup.push(v); });
        selects.forEach(s=>{ s.style.background = dup.includes(s.value) ? "#ff6b6b" : ""; });
        const hasEmpty = selects.some(s => s.value === "");
...
export function showRanking(container,data,isHost){
    ...
    if(!isHost){ html2+=`<div>あなたのチーム：${data.my_team_score ?? "-"}</div>`; }
    ...
    backBtn.onclick=()=>{ if(window.showCorrectAgain){ window.showCorrectAgain(); } };
