...
export function startNASAPlayer(ws, uiContainer) {
  ...
  if (!window.myId) { // ★URLパラメータからユーザーID取得
    const params = new URLSearchParams(location.search);
    myId = params.get("id") || null;
    window.myId = myId;
  } else {
    myId = window.myId;
  }
...
function showAdvancedMessages(data) {
  ...
  function getScoreMsg(score) {
    if (score <= 9) return "非常に優秀な判断";
    if (score <= 19) return "かなり優秀な判断";
    if (score <= 30) return "平均的な判断";
    return "要改善！";
  }

  function getDiffMsg(diff) {
    if (diff === 0) return "正確性はどちらも同じくらい";
    if (diff > 0) {
      if (diff <= 10) return "チームの方が少し良い結果";
      if (diff <= 30) return "チームで判断する方が正確！";
      return "迷ったらみんなでで話し合おう！";
    } else {
      const d = Math.abs(diff);
      if (d <= 10) return "自分の方が少し良い結果";
      if (d <= 30) return "自分で判断する方が正確！";
      return "迷ったら自分の判断を信じよう！";
    }
  }
